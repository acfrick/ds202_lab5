...
names(territories) [6] = "COUNTY"
names(territories) [7] = "County_Name"
territories$COUNTY <- as.numeric(territories$COUNTY)
territories$COUNTY = factor(territories$COUNTY)
territories$`State Code` =  as.integer(territories$`State Code`) #FIX THIS
...
```{r}
dat <- inner_join(acc, territories, by = c("COUNTY"="COUNTY", "STATE" = "State Code")) %>% select(LATITUCE, LONGITUD, COUNTY, "County_Name", DAY, MONTH, YEAR, HOUR, "State Name") #FIX THIS
```

```{r}
dat<- dat %>% mutate(County_Name = tolower(County_Name))
dat1 <- dat %>% group_by(County_Name) %>% summarise(crashes = n())
dat1 <- left_join(dat1, counties, by = "County_Name") #FIX THIS
```

```{r}
dat1 %>% group_by("County_Name") %>% ggplot(aes(x = long, y = lat, fill=crashes)) + geom_polygon(aes(group=group)) #FIX THIS
```
