...
```{r}
accdriver <- inner_join(acc, per, by = c("COUNTY", "DAY", "MONTH"))
```
...
```{r}
#ggplot(joined, aes(x= LONGITUD, y= LATITUDE)) + geom_polygon(aes(group=group))
```

Visualize the number of accidents in each county using a choropleth map. Join the accident data, GLCs, appropriate map data.

```{r}
#ggplot(joined, aes(x= )) + geom_histogram() + facet_wrap(~`County Name`)
```
