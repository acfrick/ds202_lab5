...
territories$`State Code` =  as.integer(territories$`State Code`)
...
```{r}
dat <- inner_join(acc, territories, by = c("COUNTY"="COUNTY", "STATE" = "State Code")) %>% select(LATITUCE, LONGITUD, COUNTY, "County_Name", DAY, MONTH, YEAR, HOUR, "State Name")
```

```{r}
dat<- dat %>% mutate(County_Name = tolower(County_Name))
dat1 <- dat %>% group_by(County_Name) %>% summarise(crashes = n())
dat1 <- left_join(dat1, counties, by = "County_Name")
```
